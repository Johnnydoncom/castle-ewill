...
import { listActivePlans, formatNaira } from "@/lib/actions/content";
...
export default async function PricingPage() {
  const [plans, session] = await Promise.all([listActivePlans(), auth()]);
  const signedIn = Boolean(session?.user?.id);
  const flutterwaveEnabled = flutterwaveConfigured();
