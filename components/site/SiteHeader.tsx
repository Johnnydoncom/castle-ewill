      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-2">
            <nav className="mr-2 hidden items-center lg:flex">
              {nav.map((n) => (
                <Link
                  key={n.href}
                  href={n.href}
                  className={`group relative px-3.5 py-2 text-[13px] font-medium text-muted-foreground transition-colors hover:text-navy ${pathname === n.href ? "text-navy" : ""
                    }`}
                >
                  {n.label}
            ...
            <Link
              href="/login"
              className="hidden h-11 items-center rounded-full px-5 text-[13px] font-semibold text-navy transition-colors hover:bg-muted md:inline-flex"
            >
              Sign in
            </Link>
            <Link
              href="/register"
              className="group hidden h-11 items-center gap-2 rounded-full bg-navy px-6 text-[13px] font-semibold text-navy-foreground transition-all hover:shadow-elegant md:inline-flex"
            >
              Start your Will
          ...
          <div className="mx-auto flex max-w-7xl gap-6 overflow-x-auto px-6 py-2.5">
            {nav.map((n) => (
              <Link
                key={n.href}
                href={n.href}
                className={`whitespace-nowrap text-[13px] font-medium text-muted-foreground hover:text-navy ${pathname === n.href ? "text-navy" : ""
                  }`}
              >
                {n.label}
              </Link>
